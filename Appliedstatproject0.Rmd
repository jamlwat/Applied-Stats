...
```{r}
hist(bootstrap_results$t, breaks = 30, main = "Bootstrap Means", xlab = "Mean Value", col = "lightblue", border = "black")
```
```{r}
observed_mean <- mean(df$RBI)
null_data <- df$RBI - mean(df$RBI) + 15
set.seed(123)  
boot_null <- boot(null_data, boot_mean, R = 1001)
p_value <- mean(abs(boot_null$t) >= abs(observed_mean))
print(p_value)
```
Really not sure how I am getting very high p-values.
```{r}
boot.ci(bootstrap_results, type = c("perc", "bca"))
```
The 95% confidence interval for the mean is approximately between 7.989 and 10.663 RBIs.
```{r}
hist(boot_null$t, breaks = 30, main = "Bootstrap Null Distribution",
     xlab = "Bootstrap Means (under H₀: mean = 15)", col = "lightblue")
